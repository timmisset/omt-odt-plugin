// This is a generated file. Not intended for manual editing.
package com.misset.opp.ttl.psi.impl;

import com.intellij.extapi.psi.ASTWrapperPsiElement;
import com.intellij.lang.ASTNode;
import com.intellij.psi.PsiElementVisitor;
import com.misset.opp.ttl.psi.TTLIri;
import com.misset.opp.ttl.psi.TTLRdfLiteral;
import com.misset.opp.ttl.psi.TTLString;
import com.misset.opp.ttl.psi.TTLVisitor;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

public class TTLRdfLiteralImpl extends ASTWrapperPsiElement implements TTLRdfLiteral {

  public TTLRdfLiteralImpl(@NotNull ASTNode node) {
    super(node);
  }

  public void accept(@NotNull TTLVisitor visitor) {
    visitor.visitRdfLiteral(this);
  }

  @Override
  public void accept(@NotNull PsiElementVisitor visitor) {
    if (visitor instanceof TTLVisitor) accept((TTLVisitor) visitor);
    else super.accept(visitor);
  }

  @Override
  @Nullable
  public TTLIri getIri() {
    return findChildByClass(TTLIri.class);
  }

  @Override
  @NotNull
  public TTLString getString() {
    return findNotNullChildByClass(TTLString.class);
  }

}

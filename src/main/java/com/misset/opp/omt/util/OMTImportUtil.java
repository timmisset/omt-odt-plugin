...
import com.misset.opp.omt.inspection.quickfix.OMTImportMemberLocalQuickFix;
...
public class OMTImportUtil {
    ...
    private static List<LocalQuickFix> getImportQuickFixes(OMTFile importedFile, OMTFile importingFile, PsiCall call) {
        Map<String, Collection<PsiCallable>> exportedMembers = importedFile.getExportingMembersMap();
        String callId = call.getCallId();
        if (exportedMembers.containsKey(callId)) {
            Collection<PsiCallable> psiCallables = exportedMembers.get(callId);
            return psiCallables.stream()
                    .map(psiCallable -> new OMTImportMemberLocalQuickFix(importingFile, psiCallable))
                    .collect(Collectors.toList());
        }
        return Collections.emptyList();
    }
